...
public class TruyenFullService {
    ...
    //get contents of chapter
    public Content getContentChapter(String url) throws IOException {
        Document doc = Jsoup.connect(url).timeout(5000).get();
        //get name of comic
        Element title = doc.select("a[class=truyen-title]").first();
        //get chapter
        Element chapter = doc.select("a[class=chapter-title]").first();
        //get content of a chapter
        Elements content = doc.select("div#chapter-c");
      return new Content(title.text(),chapter.text(),content.text());
    }
    ...
    // new version
    //get total chapters
    public int getTotalChapters(String url) throws IOException {
        Document doc = Jsoup.connect(url).timeout(5000).get();
        Elements pages = doc.select("ul[class=list-chapter] li");
        String endPage = pages.get(pages.size()-1).text();
        Pattern pattern = Pattern.compile("\\d+");
        Integer totalChapter = Integer.valueOf(0);
        ...
        if (matcher.find()) {
            ...
            totalChapter = Integer.valueOf(chapterNumber);
            // Output: 50
        } else {
   ...
   public ChapterDetail getDetailChapter(String idNovel, String idChapter) throws IOException {
        ...
        Elements content = doc.select("div#chapter-c");
        //get total chapter
        Elements pages = docB.select("ul[class=pagination pagination-sm] li");
        Integer totalChapters =0 ;
        if(pages.size()==0)
        {
            totalChapters = docB.select("ul[class=list-chapter] li").size();
        }
        else {
            StringBuilder linkEndPage = new StringBuilder();
            ...
            {
                linkEndPage.append(page.select("a").attr("href"));
                totalChapters = getTotalChapters(linkEndPage.toString());
            }
            else
            {
