...
const StoryHistoryRow = ({ data, color }: { data: IHistoryRoot; color: boolean }) => {
  ...
  return (
    <div
      className={`flex mb-2 text-slate-500 text-sm items-center gap-2 md:p-2 md:px-2 px-1 p-1 ${color && "bg-slate-100 dark:bg-stone-900"}`}
    >
      <div className="w-1/12 truncate hidden md:block">{convertDateToTime(data.time)}</div>
      <Link
        to={`/truyen/${data.novelId}/${data.chapterId}`}
        className="w-8/12 md:text-base text-sm font-bold text-slate-700 dark:text-neutral-300 truncate hover:text-amber-800"
      >
        {data.name}
      </Link>
      ...
      <div className="flex-1 flex justify-end">
        <button className="mr-1 border border-amber-600 text-amber-600 p-1 rounded">
          <IoCloseOutline className="text-base" onClick={() => dispatch(removeNovelReaded(data.novelId))} />
        </button>
      </div>
    </div>
  )
