  ...
  return (
    <section className={`mt-1 flex ${isEnd && 'justify-end'}`}>
      <div className='w-full md:w-7/12 flex flex-col md:flex-row items-center justify-center text-center'>
        <img
          className={`inline-flex object-cover border-4 ${border} rounded-full shadow-[5px_5px_0_0_rgba(0,0,0,1)] ${shadow} bg-indigo-50 !h-32 !w-32 mb-4 md:mb-0 ml-0 md:mr-5`}
          src={imageURL}
          alt={name}
        />
        <div className='flex flex-col'>
          <div className='md:text-justify mb-3'>
            <div className='flex flex-col mb-5'>
              <p className={`${color_1} font-bold text-xl`}>{name}</p>

              <ul className='mt-2 flex flex-row items-center justify-center md:justify-start '>
                <li className='mr-3'>
                  <a href={githubUrl} target='_blank' aria-label='GitHub' rel='noreferrer'>
                    <svg
                      className={`h-6 ${color_1} ${hover_color_2}`}
                      fill='currentColor'
                      viewBox='0 0 24 24'
                ...
                <li className='mr-3'>
                  <a href={facebookUrl} target='_blank' aria-label='Facebook' rel='noreferrer'>
                    <svg
                      className={`h-6 ${color_1} ${hover_color_2}`}
                      fill='currentColor'
                      viewBox='0 0 24 24'
