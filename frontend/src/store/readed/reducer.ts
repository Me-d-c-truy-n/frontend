...
export const readedReducer = {
  addNovelReaded(state: IReadedData, action: PayloadAction<IReadedRoot>) {
    const filterUnReaded = state.readed.filter((read: IReaded) => read.novelId !== action.payload.novelId)
    const filterReaded = state.readed.filter((read: IReaded) => read.novelId === action.payload.novelId)
    ...
    if (filterReaded.length <= 0) {
      ...
      const listChapter = filterReaded[0].chapterId.filter((chapter: string) => chapter !== action.payload.chapterId)
