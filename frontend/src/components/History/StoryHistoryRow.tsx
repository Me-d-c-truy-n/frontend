...
const StoryHistoryRow = ({ data, color }: {data: IHistoryRoot,color: boolean}) => {
  return (
    <div className={`flex mb-2 text-slate-500 text-sm items-center gap-2 p-2 px-3 ${color&&'bg-slate-100'}`}>
      <div className="w-1/12 truncate">{data.time}</div>
      <Link to={data.url} className="w-8/12 text-base font-bold text-slate-700 truncate hover:text-amber-800">{data.name}</Link>
      <div className="w-2/12">Đã đọc: {data.readed}/{data.totalChapter}</div>
