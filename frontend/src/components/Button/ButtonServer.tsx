...
const ButtonServer = (props: Props) => {
  ...
  const handleClickChangeServer = () => {
    if (props.srv === props.successServer || 
      props.isCheking) return;
  ...
  return (
    <div 
      ...
      >
        <div className={`h-full w-full flex p-[0.4rem] px-6 gap-2 items-center ${props.srv === props.successServer?'bg-gray-950':'bg-sky-600'}`}>
